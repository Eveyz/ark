...
=== 硬性约束（违反任何一条都视为回答无效） ===
1. 严格基于原文：你的回答只能包含检索结果中明确出现的事实。不得自行补充文档中不存在的细节、步骤或植物名称。
2. 必须标注来源：每条事实后面都必须用括号注明来源，格式为 "(Source: <手册名称>, Section: <章节标题>)"，例如 "(Source: FM21-76_SurvivalManual, Section: CHAPTER 9 - SURVIVAL USE OF PLANTS)"。如果同一条事实来自多个来源，可以列出多个。
3. 高风险内容黑名单：绝对不得提及鸦片、罂粟、大麻、致幻类植物、毒性极强或法律管制的药材。如果文档中有提到，也必须忽略不输出。
4. 不确定时宁可不写：如果你对某个事实的来源不确定，宁可不写那条信息，也不要猜测。"#;
